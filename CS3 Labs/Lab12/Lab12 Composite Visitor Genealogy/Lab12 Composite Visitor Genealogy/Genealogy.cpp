...
public:
    void visit(Man *m) override {
        string MiddleName = "";
        if(m->getFather() != nullptr) { // If man has a father, then continue
            MiddleName = m->getFather()->getFirstName(); // Sets MiddleName string to Fathers first name.
            // Erasing middle name that might have been previously attached to first name.
            char find = MiddleName.find_first_of(" ");
            for (int i = find; i < MiddleName.size(); ++i) {
                MiddleName.erase(i);
            }
            m->setFirstName(m->getFirstName() + " " + MiddleName); // Changes first name to be first name + middle name.
        }
    }
    
    void visit(Woman *w) override {
        string MiddleName = "";
        if(w->getMother() != nullptr) { // If woman has a mother, then continue
            MiddleName = w->getMother()->getFirstName(); // Sets MiddleName string to Mothers first name.
            // Erasing middle name that might have been previously attached to first name.
            char find = MiddleName.find_first_of(" ");
            for (int i = find; i < MiddleName.size(); ++i) {
                MiddleName.erase(i);
            }
            w->setFirstName(w->getFirstName() + " " + MiddleName); // Changes first name to be first name + middle name.
...
int main(){
    ...
    MiddleNameAdder* middles = new MiddleNameAdder;
    cout << "\nUPDATED NAME LIST\n";
    ms->accept(middles);
    ms->accept(np);
    cout << endl << endl;
